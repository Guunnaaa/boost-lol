...
def determine_playstyle(stats, role, lang_dict):
    ...
    if stats['kp'] >= 0.65: badges.append((lang_dict.get("b_kp", "Team"), "b-green"))
    if stats['dmg_min'] >= 800: badges.append((lang_dict.get("b_dmg", "Dmg"), "b-red"))
    if stats['solokills'] >= 2.5: badges.append((lang_dict.get("b_duel", "Duel"), "b-red"))
    if stats['obj'] >= 5000: badges.append((lang_dict.get("b_breach", "Breach"), "b-gold"))
    if stats['kda'] < 1.5: badges.append((lang_dict.get("b_grey", "Grey"), "b-red"))
    if stats['vis_min'] < 0.4 and role != "ADC": badges.append((lang_dict.get("b_blind", "Blind"), "b-red"))
    if stats['dmg_min'] < 300 and role not in ["UTILITY", "JUNGLE"]: badges.append((lang_dict.get("b_afk", "AFK"), "b-blue"))
    if not badges: badges.append((lang_dict.get("b_std", "Std"), "b-blue"))
    return badges[:3] 
    ...
    if "#" not in riot_id_input:
        ...
        with st.spinner(T["loading"]):
            ...
            if best_duo and max_g >= 2:
                ...
                # --- APPEL CORRECT DES FONCTIONS ---
                badges_me = determine_playstyle(avg_me, role_me, T)
                badges_duo = determine_playstyle(avg_duo, role_duo, T)
                qual, flaw = analyze_qualities(stats=avg_me, role=role_me, lang_dict=T)
                qual_d, flaw_d = analyze_qualities(stats=avg_duo, role=role_duo, lang_dict=T)
