...
# --- CONFIGURATION ---
st.set_page_config(page_title="LoL Duo Analyst V71", layout="wide")
...
TRANSLATIONS = {
    "FR": {
        ...
        "b_kda": "🛡️ KDA Player", "b_vis": "👁️ Oracle", "b_kp": "🤝 Teamplayer", "b_dmg": "⚔️ Machine", "b_duel": "🩸 Dueliste", "b_breach": "🏰 Destructeur", "b_grey": "👻 Écran Gris", "b_blind": "🕶️ Aveugle", "b_afk": "💤 AFK Farm", "b_std": "⚖️ Standard"
    },
    "EN": {
...
# --- FONCTION DETERMINE PLAYSTYLE (RESTAURÉE) ---
def determine_playstyle(stats, role, lang_dict):
    """Détermine des badges basés sur les stats par minute"""
    badges = []
    # Badges positifs
    if stats['kda'] >= 4.0: badges.append((lang_dict.get("b_kda", "KDA"), "b-gold"))
    if stats['vis_min'] >= 2.0 or (role == "UTILITY" and stats['vis_min'] >= 2.5): badges.append((lang_dict.get("b_vis", "Oracle"), "b-blue")) 
    if stats['kp'] >= 0.65: badges.append((lang_dict.get("b_kp", "Teamplayer"), "b-green"))
    # Badges agressifs / Carry
    if stats['dmg_min'] >= 800: badges.append((lang_dict.get("b_dmg", "Machine"), "b-red"))
    if stats['solokills'] >= 2.5: badges.append((lang_dict.get("b_duel", "Duelist"), "b-red"))
    if stats['obj'] >= 5000: badges.append((lang_dict.get("b_breach", "Breacher"), "b-gold"))
    # Badges négatifs
    if stats['kda'] < 1.5: badges.append((lang_dict.get("b_grey", "Grey Screen"), "b-red"))
    if stats['vis_min'] < 0.4 and role != "ADC": badges.append((lang_dict.get("b_blind", "Blind"), "b-red"))
    if stats['dmg_min'] < 300 and role not in ["UTILITY", "JUNGLE"]: badges.append((lang_dict.get("b_afk", "AFK Farm"), "b-blue"))

    if not badges: badges.append((lang_dict.get("b_std", "Standard"), "b-blue"))
    return badges[:3] 

# --- FONCTION ANALYSE QUALITÉS (REST.) ---
    ...
    if "#" not in riot_id_input:
        ...
        with st.spinner(T["loading"]):
            ...
            if best_duo and max_g >= 2:
                ...
                elif ratio > 1.15: title, color, sub = T["v_lead"], "#00BFFF", safe_format(T["s_lead"], target_name, duo_name)
                ...
                qual_me, flaw_me = analyze_qualities(stats_me, role_me, T)
                qual_duo, flaw_duo = analyze_qualities(stats_duo, role_duo, T)
                ...
                def display_player_card(name, champs, stats, badges, role_icon, diff_stats, color_theme, q, f):
                    ...
                    st.markdown(f"""<div class="player-card" style="border-top: 4px solid {color_theme};"><div class="player-name">{name}</div><div class="player-sub">{role_icon}</div>{fb_html}<div style="margin:15px 0;">{badges_html}</div><div style="margin-bottom:20px;">{champs_html}</div>{stat_grid_html}</div>""", unsafe_allow_html=True)
