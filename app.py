...
<style>
    ...
    .stat-lbl {{ font-size: 11px; color: #999; text-transform: uppercase; margin-top: 4px; font-weight: 600; }}
...
def analyze_qualities(stats, role, lang_dict):
    qualities, flaws = [], []
    if stats['kda'] >= 4.0: qualities.append((lang_dict.get("q_surv", "KDA Player"), "b-gold"))
    if stats['vis_min'] >= 2.0: qualities.append((lang_dict.get("q_vis", "Oracle"), "b-blue")) 
    if stats['kp'] >= 0.65: qualities.append(("Teamplayer", "b-green"))
    if stats['dmg_min'] >= 800: qualities.append((lang_dict.get("q_dmg", "Heavy Hitter"), "b-red"))
    if stats['solokills'] >= 2.5: qualities.append(("Duelist", "b-red"))
    if stats['obj'] >= 5000: qualities.append((lang_dict.get("q_obj", "Breacher"), "b-gold"))
    
    if stats['kda'] < 1.5: badges = [("Grey Screen", "b-red")]
    elif stats['dmg_min'] < 300 and role not in ["UTILITY", "JUNGLE"]: badges = [("AFK Farm", "b-blue")]
    else: badges = qualities if qualities else [("Standard", "b-blue")]
    return badges[:3]
