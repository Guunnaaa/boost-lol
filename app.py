...
# --- CONFIGURATION ---
st.set_page_config(page_title="LoL Duo Analyst V74", layout="wide")
...
st.markdown(
    ...
    .stButton > button {{ width: 100%; height: 55px; background: linear-gradient(135deg, #ff0055, #cc0044); color: white; font-size: 18px; font-weight: 800; border: none; border-radius: 8px; text-transform: uppercase; letter-spacing: 1px; -webkit-appearance: none; appearance: none; }}
...
def analyze_qualities(stats, role, lang_dict):
    ...
    # --- DÉFAUTS (KEY FIX: dmg_min & gold_min) ---
    scores = {
        'kda': stats['kda'] / 3.0,
    ...
    if role != "UTILITY":
        scores['dmg_min'] = stats['dmg_min'] / 500.0
        scores['gold_min'] = stats['gold_min'] / 400.0
    ...
    if not riot_id_input or "#" not in riot_id_input:
        ...
        with st.spinner(T["loading"]):
            ...
            if best_duo and max_g >= 2:
                ...
                # Rename keys for logic (dpm -> dmg_min, gold -> gold_min)
                stats_me = {'kda': avg_f(s_me, 'kda'), 'dmg_min': avg(s_me, 'dpm'), 'vis_min': avg(s_me, 'vis'), 'obj': avg(s_me, 'obj'), 'gold_min': avg(s_me, 'gold')}
                stats_duo = {'kda': avg_f(s_duo, 'kda'), 'dmg_min': avg(s_duo, 'dpm'), 'vis_min': avg(s_duo, 'vis'), 'obj': avg(s_duo, 'obj'), 'gold_min': avg(s_duo, 'gold')}
                ...
                with col_left:
                    ...
                    render_stat_row("DPM", stats_me['dmg_min'], stats_me['dmg_min'] - stats_duo['dmg_min'])
                    render_stat_row("GOLD", stats_me['gold_min'], stats_me['gold_min'] - stats_duo['gold_min'])
                    render_stat_row("VISION", stats_me['vis_min'], stats_me['vis_min'] - stats_duo['vis_min'])
                ...
                with col_right:
                    ...
                    render_stat_row("DPM", stats_duo['dmg_min'], stats_duo['dmg_min'] - stats_me['dmg_min'])
                    render_stat_row("GOLD", stats_duo['gold_min'], stats_duo['gold_min'] - stats_me['gold_min'])
                    render_stat_row("VISION", stats_duo['vis_min'], stats_duo['vis_min'] - stats_me['vis_min'])
