...
# --- TRADUCTIONS (TOUTES LES CLES SONT PRESENTES PARTOUT) ---
TRANSLATIONS = {
    "FR": {
    ...
    "ES": {
        ...
        "v_hyper":"MVP TOTAL","s_hyper":"Domina a {duo}",
        "v_lead":"LIDER TECNICO","s_lead":"{target} lidera, {duo} sigue",
        "v_tactician":"ESTRATEGA","s_tactician":"Macro para {duo}",
        "v_fighter":"GLADIADOR","s_fighter":"Daño",
        "v_solid":"DUO SOLIDO","s_solid":"Sinergia con {duo}",
        "v_passive":"PASIVO","s_passive":"Seguro",
        "v_supp":"APOYO","s_supp":"{target} ayuda",
        "v_struggle":"DIFICULTAD","s_struggle":"Sufre vs {duo}",
    ...
    "KR": {
        "title":"LoL 듀오 분석","btn_scan":"분석 시작","placeholder":"예: Hide on bush#KR1","label_id":"Riot ID","lbl_region":"지역","lbl_mode":"모드","dpm_btn":"dpm.lol 확인","lbl_duo_detected":"{duo} 와 듀오",
        
        "v_hyper":"하드 캐리","s_hyper":"{target} > {duo}",
        "v_lead":"기술적 리더","s_lead":"{target} 리드",
        "v_tactician":"전략가","s_tactician":"운영",
        "v_fighter":"전투광","s_fighter":"딜",
        "v_solid":"완벽 듀오","s_solid":"{target} & {duo}",
        "v_passive":"버스","s_passive":"안전",
        "v_supp":"서포트","s_supp":"{target} 보조",
        "v_struggle":"고전 중","s_struggle":"역부족",
...
<style>
    ...
    .block-container {{
        max-width: 1400px !important; padding: 1rem !important; margin: auto !important;
        ...
        margin-top: 20px !important; margin-bottom: 60px !important;
    }}
    ...
    .player-panel {{ background: rgba(255, 255, 255, 0.03); border-radius: 16px; padding: 20px; height: 100%; border: 1px solid rgba(255,255,255,0.05); margin-bottom: 10px; }}
    .player-name {{ font-size: 28px; font-weight: 900; color: white; text-align: center; margin-bottom: 2px; }}
    
    .role-badge {{ font-size: 12px; font-weight: 700; color: #aaa; text-align: center; margin-bottom: 12px; letter-spacing: 1px; opacity: 0.8; }}
    ...
    .stButton > button {{ width: 100%; height: 60px; background: linear-gradient(135deg, #ff0055, #cc0044); color: white; font-size: 20px; font-weight: 800; border: none; border-radius: 10px; text-transform: uppercase; transition: 0.3s; box-shadow: 0 0 15px rgba(255, 0, 85, 0.3); }}
